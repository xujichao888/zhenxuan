<template>
  <div class="jx-products" v-if="isIn">
    <div class="jx-products-introduced jx-padding-left-right-20 jx-margin-bottom-20">
      ...
      <div class="jx-products-text jx-text-displays-ellipses">
        {{productsInfo.product_name}}
      </div>
      <div class="jx-products-price">
        <div class="jx-new-price">
          <span style="font-size: 26rpx;">￥{{selectGoods.goodsMoney}}</span>
          <span style="font-size: 36rpx;font-weight: bold;margin-left: 5rpx;">{{productsInfo.newPrice}}</span>
          <span class="jx-radius-pink" v-if="productsInfo.discounts">{{productsInfo.discounts}}折</span>
          <span class="jx-radius-pink">七天无理由退货</span>
        </div>
        <div class="jx-old-price" v-if="productsInfo.pre_price">
          原价
          <span style="text-decoration:line-through;">￥{{productsInfo.pre_price/100}}</span>
        </div>
      </div>
    </div>
    <div class="jx-padding-left-right-20 jx-margin-bottom-20 jx-products-specification">
      ...
      <div class="jx-color-specification" v-for="(k, __index) in productsInfo.attributeInfos" :key="__index">
        <div>
          <span style="margin-left: 8rpx;font-weight: 400;">
            {{k.text}}
          </span>
        </div>
      ...
      <div class="jx-specification-count">
        <div style="margin-left: 8rpx;font-weight: 400;">购买数量</div>
      ...
      this.attributeInfos.map(item => {
        if (this.selectSpecification.hasOwnProperty(item.attr_id1) && this.selectSpecification.hasOwnProperty(item.attr_id2)) {
          if (item.value_1 === this.selectSpecification[item.attr_id1] && item.value_2 === this.selectSpecification[item.attr_id2]) {
            ...
            console.log(this.productsInfo.shortage, 'this.productsInfo.shortage')
          }
        } else {
          ...
          this.$http.get({
            ...
            if (res.code * 1 === 0) {
              ...
              if (isLeader != '' && isLeader == 2) {
                wx.redirectTo({ url: `/pages/products-leader/main?productsId=${this.productsId}` });
                return
              }
              this.getOrdercardList()
            } else {
